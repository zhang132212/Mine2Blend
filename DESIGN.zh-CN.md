...
- 每 16³ chunk 一个派生 mesh，边界编辑使相关 3×3×3 chunk 失效。当前仍是合并网格，尚无 GN 实例化后端。
...
| 4 | 渲染 | 分块、模型缓存、atlas、cutout/alpha、tint、AO、静态流体 | GN 实例化；实体预览；动态旗帜图案/告示牌文字/皮肤；动画 |
...
| 13 | 测试 | Python 单测、Blender 后台流程、47 CTM 图块、GUI 画笔放置；三平台 CI 配置 | CI 实际通过；游戏端对照；新版安装包与 GUI 最终复验 |
